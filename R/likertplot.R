  ...
  if(is.null(data)){
   ...
   if (is.data.frame(data) ) {
     if("plot" %in% names(attributes(data))){
       ...
       # Hier kommen daten im Breit-Format 
       formula <- x
       my_vars <- all.vars(x)
       if(!("."  %in% my_vars)) stop( "Ich kann nur mit aufbereiteten Daten und Item ~. arbeiten!)")
       name_item <- my_vars[1]
       nlevels<- length(names(data)) - length(my_vars) + 1
       measure <-  setdiff( names(data), my_vars)
       x_mean <-  colSums(t(data[measure])* seq_len(nlevels), na.rm=TRUE ) 
     }
   }
    else if( inherits(x, "likert") ){
       formula <- x

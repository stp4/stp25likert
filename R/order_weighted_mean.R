 ...
 # cat("Aha du willst also sortieren.\n")
  ...
  if (!is.null(dat_attr)) {
    ...
    if(attr(data, "likert") == "long") {
      data_long <- data
      data <- attr(data, "data_wide")
      data_wide <- FALSE
    }
  }
 # cat("  Kommt ein wide-data.frame:" , data_wide)
  
  if (is.character(include.order)) {
    orrr <- include.order <- match.arg(include.order, c("right", "left"))
  ...
  if (length(groups) > 0) {
    m_weight <- tapply(m_weight, data[[items]],
                       FUN = function(x) {mean(x, na.rm = TRUE)})
  }
  else{
    names(m_weight) <- as.character(data[[items]])
  }
  ...
  if (data_wide) {
    ...
    data$ordr_weight <- as.numeric(data[[items]])
    dplyr::arrange(data, desc(ordr_weight))[-ncol(data)]
  }
  else{

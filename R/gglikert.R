...
gglikert <- function(data,
                     ...
                     # customised Mar 17 08:46:33 2024
                     add_totals = FALSE, 
                     totals_size = labels_size*.9,
                     totals_color = "black",
  ...
  if (add_totals) {
   ...
   # dtot$.answer<- levels(p$data$.answer)[1]
   #  p$data <- 
   #    dplyr::left_join(p$data, dtot, by= c("Item", ".answer")  )
    p$data <- 
       dplyr::left_join(p$data, dtot, by= c("Item") ,
...
gglikert_data <- function(data,
                          ...
                          exclude_fill_values = NULL) {
  # not necessary for me Mar 17 17:32:14 2024"
  ...
  data$.question <- data_labels[data$.question] |>
    forcats::fct_inorder()
  # wp  Mar 17 17:31:07 2024
  # my default is none and i don't use sort
  if (sort == "none") {
  }
  else if (sort == "ascending" && sort_method == "prop") {
  ...
  else if (sort == "descending" && sort_method == "prop") {
    data$.question <- data$.question |>
      forcats::fct_reorder2(
        data$.answer,
        data$.weights,
  ...
  else  if (sort == "ascending" && sort_method == "mean") {
    data$.question <- data$.question |>
      forcats::fct_reorder2(
        data$.answer,
        data$.weights,
  ...
  else if (sort == "descending" && sort_method == "mean") {
    data$.question <- data$.question |>
      forcats::fct_reorder2(
        data$.answer,
        data$.weights,
  ...
  else if (sort == "ascending" && sort_method == "median") {
    data$.question <- data$.question |>
      forcats::fct_reorder2(
        data$.answer,
        data$.weights,
  ...
  else if (sort == "descending" && sort_method == "median") {
    data$.question <- data$.question |>
      forcats::fct_reorder2(
        data$.answer,
        data$.weights,
